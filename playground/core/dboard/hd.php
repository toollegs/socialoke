...
function paintMsgAll()
{
	global $fullVenue;

	$fn = 'msgcnt.'.$fullVenue;
	$f = file('/var/www/html/beta/core/dboard/'.$fn);
	if (count($f) < 2) {
		echo '<div id="sendall"><input type="text" id="allmsg" maxlength="100" size="40"/><input id="sendAllBtn" style="display:inline;" type="button" value="Message All"/></div>';
	} else {
		echo 'The Message All feature is now disabled.';
	}
}
		...
		<div id="queue" <?php if (!isset($code) || $code == '') { ?>style="visibility:hidden;"<?php } ?>>
			...
			<?php 
				...
				foreach($f as $row) { 
					...
					$singerForTD = $singer;
					if (strlen($singer) > 14) {
						$singerForTD = substr($singer,0,13);
						$singerForTD .= "<br/>";
						$singerForTD .= substr($singer,13);
					}
			...
			?>
				<tr id="<?php echo $i ?>" style='font-size: 12px;'>
					<td><?php echo $singerForTD ?></td>
					<td><?php echo firstlast($song).' by '.firstlast($artist) ?></td>
					<td><?php echo $waittime ?></td>
					<td><?php echo $note ?></td>
					<td><?php echo $theseVotes; ?></td>
			<?php if ($pn !== null && $pn !== '' && !is_numeric($pn)) { ?><td><a href="<?php echo $noteKey ?>" target="_blank">Interact</a><?php } else { ?><td>Coming Soon<?php } ?></td>
...
$(document).ready(function(){
	...
	$("#sendAllBtn").click(function(event) {
		var msg = $("#allmsg").val();
		$("#sendall").css("display","none");
		$.ajax({
  			type: "GET",
			url: "/beta/core/msgng/msgall.php?m="+msg+"&fv=<?php echo $fullVenue ?>",
			success: function (dataCheck) {
				if (dataCheck != 'Message All Expired!') {
	                		$('#gigstatus').text("Message Sent!");
				} else {
	                		$('#gigstatus').text(dataCheck);
				}
			}
		});
	});
